...
from opacus.optimizers import DCSGDPOptimizer
from opacus.optimizers import DCSGDEOptimizer
        ...
        **kwargs,
        ...
        if clipping == "dcsgdp":
            return DCSGDPOptimizer(
                ...
                generator=generator,
                secure_mode=self.secure_mode, #从此处开始后面的参数都不会在新方法中被调用
                batchsize_train=kwargs.get("batchsize_train", expected_batch_size),
                dimension=kwargs.get("dimension", sum(p.numel() for p in optimizer.param_groups[0]["params"])),
                percentile=kwargs.get("percentile", 0.3),
                stride=kwargs.get("stride", 2.0),
                bin_cnt=kwargs.get("bin_cnt", 20),
            )
        
        elif clipping == "dcsgde":
            #expected_batch_size = kwargs.get("expected_batch_size", None)
            optimizer = DCSGDEOptimizer(
                optimizer,
                noise_multiplier=noise_multiplier,
                max_grad_norm=max_grad_norm,
                histogram_std=kwargs.get("histogram_std", 6.0),
                expected_batch_size=expected_batch_size,
