        ...
        if not args.disable_dp:
            ...
            if max_grad_norm is not None:
                print(
                    ...
                    f"Clip Norm C = {max_grad_norm:.4f}"
                )
            else:
                print(
                    f"\tTrain Epoch: {epoch} \t"
...
def main():
    ...
    if not args.disable_dp:
        ...
        # 计算 model 参数总维度，用于传给 DCSGDPOptimizer
        total_dimension = sum(p.numel() for p in model.parameters())

        # 决定使用哪种 clipping 策略
        if args.clipping == "per_layer":
            n_layers = len(
                [(n, p) for n, p in model.named_parameters() if p.requires_grad]
            )
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        # 记录 epsilon 和 C（Clipping threshold）
        if not args.disable_dp:
            epsilon = privacy_engine.accountant.get_epsilon(delta=args.delta)
            epsilon_per_epoch.append(epsilon)

            # DCSGDP 优化器记录 clip 值
            if args.clipping == "dcsgdp" and hasattr(optimizer, "current_clip"):
                clip_norm_per_epoch.append(optimizer.current_clip)
            elif hasattr(optimizer, "max_grad_norm"):
                clip_norm_per_epoch.append(
                    optimizer.max_grad_norm[0] if isinstance(optimizer.max_grad_norm, list)
                    else optimizer.max_grad_norm
                )
    ...
    if world_size > 1:
        cleanup()
    if not args.disable_dp and len(epsilon_per_epoch) > 0:
        ...
        ax1.plot(range(1, len(clip_norm_per_epoch) + 1), clip_norm_per_epoch, color="tab:red", label="Clipping Threshold C")
        ax1.tick_params(axis="y", labelcolor="tab:red")

        ax2 = ax1.twinx()
        ax2.set_ylabel("Epsilon", color="tab:blue")
        ax2.plot(range(1, len(epsilon_per_epoch) + 1), epsilon_per_epoch, color="tab:blue", linestyle="--", label="Epsilon")

        if len(epsilon_dynamic_per_epoch) > 0:
            ax2.plot(range(1, len(epsilon_dynamic_per_epoch) + 1),
                    epsilon_dynamic_per_epoch, color="tab:green", linestyle=":", label="Epsilon (Dynamic C)")

        ax2.legend(loc="upper left")
        ax2.tick_params(axis="y", labelcolor="tab:blue")

        plt.title("Clipping Threshold and Epsilon Over Epochs (CIFAR10)")
        fig.tight_layout(rect=[0, 0, 1, 0.95])
        plt.grid(True)
        plt.savefig("clip_and_epsilon_cifar10.png")
        plt.show()
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="flat",
        choices=["flat", "per_layer", "dcsgdp"],
        help="Clipping strategy: flat, per_layer, or dcsgdp",
    )
    parser.add_argument(
        "--epochs",
        default=10,
